...
clusters = [x for x in clust["cluster_id"].unique()]

ncol = len(samples) * len(clusters)
...
for cluster in np.sort(clusters):
    ...
    L.info("Working on cluster: " + str(cluster))
    ...
    barcodes_to_fetch = clust.index[clust["cluster_id"] == cluster]
    ...
    for sample in samples:
           ...
           col_names.append(str(cluster) + "_" + str(sample))
           col_idx += 1

L.info("Summation of counts complete")
pseudocount_frame = pd.DataFrame(pseudocounts,
                                 columns = col_names,
                                 index = [x for x in x.var["gene_ids"].values])

pseudocount_frame = pseudocount_frame[col_names].astype(int)

pseudocount_frame["gene_id"] = pseudocount_frame.index

pseudocount_frame = pseudocount_frame[["gene_id"] + col_names]
