# ------------------------------------------------------------------------------
# CLR normalized ADT UMAP embedding & visualization 
...
option_list <- list(
  ...
  make_option(c("--npcs"), default=15,
              help="Number of top PCs to consider for UMAP embedding."),
  ...
  make_option(c("--outfile"), default="fetch.cells.dir/ADT.mtx.full.dir/clr_umap_embedding.pdf")
)
...
pdf(gsub("\\.pdf", paste0("_UMI_protein_variation.pdf"), opt$outfile),
    height = 4, width = 8)
...
adt_clr_mat <- data.matrix(adt@assays$RNA@data)
write.table(adt_clr_mat, gsub("\\.pdf", 
                              paste0("_CLR_norm_matrix.tsv"), 
                              opt$outfile), 
            sep = "\t", quote = FALSE, row.names = TRUE)

adt@assays$RNA@var.features <- rownames(adt)

adt <- ScaleData(adt) 

# -- PCA embedding of CLR normalized ADT expression ----------------------------

adt <- RunPCA(adt,
              reduction.name = 'pca',
              npcs = length(genes))

pdf(gsub("\\.pdf", "_CLR_PCA_elbow.pdf", opt$outfile),
    height = 3, width = 8)

  ElbowPlot(adt, reduction = "pca", ndims = 50)

dev.off()

# -- UMAP embedding of CLR-normalized-based PCA --------------------------------
...
umap_clr <- data.frame(adt@reductions$umap@cell.embeddings)
umap_clr[["barcode_id"]] <- rownames(umap_clr)

write.table(umap_clr, gsub("\\.pdf", 
                           paste0("_CLR_pca_umap_barcode_embedding.tsv"), 
                           opt$outfile), 
...
png(gsub("\\.pdf", "_umap_clr_protein.png", opt$outfile),
...
ggs <- lapply(rownames(adt), function(gene) plot_adt_feat(gene, adt, umap_clr))

nc <- ceiling(sqrt(length(ggs)))

png(gsub("\\.pdf", paste0("_umap_clr_proteins.png"), opt$outfile),
    height = 800*nc, width = 800*nc, res = 300)

  plot_grid(plotlist = ggs, nrow = nc)

dev.off()

# -- ADT count distribution ----------------------------------------------------

sample_ids <- unique(meta[[opt$colorvar]])

gg_umi <- lapply(sample_ids, function(s) {
  b <- filter(meta, !!rlang::sym(opt$colorvar) == s)[["barcode_id"]]
  sub_dsb <- data.matrix(adt_mtx[, b])
  dsb <- data.frame(melt(sub_dsb))
  print(head(dsb))
  ggplot(dsb, aes(x = value+1, y = Var1, fill = Var1)) +
...
nc <- ceiling(sqrt(length(gg_umi)))

pdf(gsub(".pdf", paste0("_UMI_density_", opt$colorvar, ".pdf"), opt$outfile), 
    width = 4*nc, height = 0.15*nrow(adt_mtx)*nc)

  plot_grid(plotlist = gg_umi, ncol = nc)
...
gg_clr <- lapply(sample_ids, function(s) {
  b <- filter(meta, !!rlang::sym(opt$colorvar) == s)[["barcode_id"]]
  sub_dsb <- data.matrix(adt@assays$RNA@data[, b])
  dsb <- data.frame(melt(sub_dsb))
  ggplot(dsb, aes(x = value, y = Var1, fill = Var1)) +
    ...
    xlab("CLR normalized value") +
    theme(legend.position = "none") +
    ggtitle(s)
})

nc <- ceiling(sqrt(length(gg_clr)))

pdf(gsub(".pdf", paste0("_CLR_density_", opt$colorvar, ".pdf"), opt$outfile), 
    width = 3*nc, height = 0.12*nrow(adt_mtx)*nc)

  plot_grid(plotlist = gg_clr, ncol = nc)

dev.off()

# heatmap per colorvar ---------------------------------------------------------

dsb_mat <- data.frame(t(data.matrix(adt@assays$RNA@data)))
...
flog.info("Mean feature per sample:", mat[1:3, 1:3], capture = TRUE)

pdf(gsub(".pdf", paste0("_CLR_heatmap_", opt$colorvar, ".pdf"), opt$outfile),
    width = 2+(0.3*nrow(mat)), height = 0.3*ncol(mat))
...
pheatmap::pheatmap(t(mat[, gsub("_", ".", genes)]), 
                   scale = "row",
                   fontsize_row = 6, 
                   border_color = NA,
                   na.rm = TRUE,
