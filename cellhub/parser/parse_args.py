...
DEFAULT_MEM = 4000  # Default memory: 4000 MB.
...
if __name__ == "__main__":
    ...
    assert parse_mem(None) == 4000, f"parsed result: {parse_mem(None)}"
    assert parse_mem(False) == 4000, f"parsed result: {parse_mem(False)}"
    assert parse_mem(" falsE") == 4000, f"parsed result: {parse_mem(' falsE')}"
    assert parse_mem("nOne ") == 4000, f"parsed result: {parse_mem('nOne ')}"
    assert parse_mem(" defaulT ") == 4000, f"parsed result: {parse_mem(' defaulT ')}"
    assert parse_mem("5.3G") == 5000, f"parsed result: {parse_mem('5.3G')}"
    assert parse_mem("5300 m") == 5300, f"parsed result: {parse_mem(' 5300 m ')}"
    assert parse_mem(" 4.6 g ") == 5000, f"parsed result: {parse_mem(' 4.6 g ')}"

    print("arg_parsers.py passed the test.")
